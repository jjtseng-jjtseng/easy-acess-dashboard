...
A personal, local-only time board for the apps and sites you use most. Windows, Python 3, standard library only.
...
GitHub Releases provides a Windows x64 ZIP named Time-Board-win-x64-vX.Y.Z.zip. Extract the complete
Time-Board folder and run Time-Board.exe; the _internal folder beside it is required.
...
The legacy folder name is retained so existing installs keep their recorded history after the Time Board rename.
